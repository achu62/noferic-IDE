...
export function initiateterminal(e, id, document) {
	...
	tab.addEventListener("click", (e) => {
		e.stopPropagation();
		document.querySelectorAll(".terminaltabparallels").forEach(terminaltab => {
	...
	terminal.onData((data) => window.ipc.send("data", JSON.stringify(
		{
			action: "tdata",
			id: id,
			data: data.toString()
	...
	async function runresizeevent(){
			setInterval(()=>{
				window.ipc.send("data", JSON.stringify(
					{
						action: "tdata",
						a2:"resize",
						data: {
	...
	function setupTerminalHr(term, ptyProcess) {
		// Keeps track of when a command was sent to the shell
		let commandExecuting = false;

		// 1. Detect when the user executes a command (presses Enter)
		term.onData(data => {
			if (data === '\r' || data === '\n') {
				const line = '─'.repeat(term.cols);

				// Write a clean, dim-gray horizontal rule into the terminal buffer
